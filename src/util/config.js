...
global.daemon_rpc_ws = `wss://${self_hostname}:22200`;
...
function loadConfig(net) {
  try {
    // check if WHEAT_ROOT is set. it overrides 'net'
    const config_root_dir =
      'WHEAT_ROOT' in process.env
        ? process.env.WHEAT_ROOT
        : path.join(os.homedir(), '.wheat', net);
    ...
    const daemon_port = lodash.get(config, 'ui.daemon_port', 22200); // jshint ignore:line
