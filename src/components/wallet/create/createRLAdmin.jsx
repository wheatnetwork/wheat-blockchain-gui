...
import { AlertDialog } from '@wheat/core';
import {
  Typography,
  Button,
  Box,
...
import { wheat_to_mojo } from '../../../util/wheat';
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      wheatper_input.value === '' ||
      Number(wheatper_input.value) === 0 ||
      !Number(wheatper_input.value) ||
      isNaN(Number(wheatper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const wheatper = wheat_to_mojo(wheatper_input.value);
    const wheatper_value = Number.parseInt(Number(wheatper));
    const userpubkey = userpubkey_input.value;
    const amount = wheat_to_mojo(amount_input.value);
    const amount_value = Number.parseInt(Number(amount));
    // var fee = wheat_to_mojo(fee_input.value);
