...
import { AlertDialog, Card, Flex } from '@wheat/core';
import {
  send_transaction,
...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const wheatper_input = ip_parsed.limit;
    ...
    const wheatper_value = Number.parseInt(Number(wheatper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (wheat per interval):{' '}
                  {mojo_to_wheat_string(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const fee = wheat_to_mojo(fee_input.value);
