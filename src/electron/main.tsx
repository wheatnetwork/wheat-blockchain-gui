...
import wheatEnvironment from '../util/wheatEnvironment';
import wheatConfig from '../util/config';
...
if (!handleSquirrelEvent()) {
  ...
  const ensureCorrectEnvironment = () => {
    ...
    if (!wheatEnvironment.guessPackaged() && !('VIRTUAL_ENV' in process.env)) {
  ...
  if (ensureSingleInstance() && ensureCorrectEnvironment()) {
    ...
    wheatConfig.loadConfig('mainnet');
    ...
    const createWindow = () => {
      ...
      if (!wheatConfig.manageDaemonLifetime()) {
      ...
      mainWindow.on('close', (e) => {
        ...
        if (decidedToClose || !wheatConfig.manageDaemonLifetime()) {
          return;
        }
        e.preventDefault();
        if (!isClosing) {
    ...
    const appReady = async () => {
      ...
      // if the daemon isn't local we aren't going to try to start/stop it
      if (wheatConfig.manageDaemonLifetime()) {
        wheatEnvironment.startWheatDaemon();
      }
    };
  ...
  const getMenuTemplate = () => {
        ...
        submenu: [
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Wheat-Network/wheat-blockchain/wiki',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Wheat-Network/wheat-blockchain/wiki/FAQ',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Wheat-Network/wheat-blockchain/releases',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Wheat-Network/wheat-blockchain/blob/master/CONTRIBUTING.md',
              );
            },
          },
          {
          ...
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Wheat-Network/wheat-blockchain/issues',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal('https://keybase.io/team/wheat_network.public');
            },
          },
          {
            label: i18n._(/* i18n */ { id: 'Follow on Twitter' }),
            click: () => {
              openExternal('https://twitter.com/wheat_project');
            },
          },
        ],
      },
    ...
    if (process.platform === 'darwin') {
      // Wheat Blockchain menu (Mac)
      template.unshift({
        label: i18n._(/* i18n */ { id: 'Wheat' }),
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'About Wheat Blockchain' }),
            click: () => {
              openAbout();
            },
          },
