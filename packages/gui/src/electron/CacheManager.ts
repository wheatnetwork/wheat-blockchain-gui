...
const log = debug('wheat-gui:CacheManager');
...
const FILE_SUFFIX = '-wheatcache';
const MAX_TOTAL_SIZE = 1024 * 1024 * 1024; // 1GB
const MAX_FILE_SIZE = 1024 * 1024 * 100; // 100MB

const SUFFIXES = [FILE_SUFFIX, `${FILE_SUFFIX}${INFO_SUFFIX}`];

function isWheatCacheFile(filePath: string) {
  return SUFFIXES.some((suffix) => filePath.endsWith(suffix));
}

function isWheatCacheInfoFile(filePath: string) {
  return isWheatCacheFile(filePath) && filePath.endsWith(INFO_SUFFIX);
}
...
export default class CacheManager extends EventEmitter {
  ...
  private async removeOldestFiles(targetSize: number): Promise<void> {
    ...
    const filePaths = files
      .filter((file) => isWheatCacheFile(file) && !isWheatCacheInfoFile(file))
  ...
  async getCacheSize() {
    const files = await fs.readdir(this.cacheDirectory);
    const filePaths = files
      .filter((filename) => isWheatCacheFile(filename))
