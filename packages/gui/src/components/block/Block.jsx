...
import { useGetBlockQuery, useGetBlockRecordQuery } from '@wheat-network/api-react';
import {
  Back,
  Button,
  Card,
...
export default function Block() {
  ...
  const poolReward = mojoToWheat(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojoToWheat(calculateBaseFarmerReward(blockRecord.height));

  const wheatFees = blockRecord.fees !== undefined ? mojoToWheat(blockRecord.fees) : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: wheatFees ? `${wheatFees} ${currencyCode}` : '',
  ...
  return (
    <LayoutDashboardSub>
      <Card
        title={
          <Back variant="h5">
            <Trans>Block at height {blockRecord.height} in the Wheat blockchain</Trans>
          </Back>
        }
        action={
          <Flex gap={1}>
