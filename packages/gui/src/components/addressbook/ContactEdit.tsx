// import { useGetNFTInfoQuery } from '@wheat-network/api-react';
import { fromBech32m } from '@wheat-network/api';
import { AddressBookContext, Color, EmojiAndColorPicker, Flex, Form, TextField } from '@wheat-network/core';
...
export default function ContactEdit() {
  ...
  function handleSubmit(data) {
    ...
    filteredAddresses.forEach((entry) => {
      try {
        if (entry.address[3] === '1') {
          if (entry.address.slice(0, 5).toLowerCase() !== 'wheat') {
            throw new Error();
          } else if (fromBech32m(entry.address).length !== 64) {
            throw new Error();
          }
        } else if (entry.address[6] === '1') {
          if (entry.address.slice(0, 6).toLowerCase() !== 'twheat') {
            throw new Error();
          } else if (fromBech32m(entry.address).length !== 64) {
            throw new Error();
          }
    ...
    filteredProfiles.forEach((entry) => {
      try {
        if (entry.did.slice(0, 11).toLowerCase() !== 'did:wheat:') {
          throw new Error();
