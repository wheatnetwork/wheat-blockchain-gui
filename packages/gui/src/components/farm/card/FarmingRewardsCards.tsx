import { useGetBlockchainStateQuery, useGetTotalHarvestersSummaryQuery } from '@wheat-network/api-react';
import { State, CardSimple, useCurrencyCode, mojoToWheatLocaleString, useLocale } from '@wheat-network/core';
...
const MOJO_PER_WHEAT = 1_000_000_000_000;
const BLOCKS_PER_YEAR = 1_681_920; // 32 * 6 * 24 * 365
function getBlockRewardByHeight(height: number) {
  if (height === 0) {
    return 21_000_000 * MOJO_PER_WHEAT;
  }
  if (height < 3 * BLOCKS_PER_YEAR) {
    return 2 * MOJO_PER_WHEAT;
  }
  if (height < 6 * BLOCKS_PER_YEAR) {
    return 1 * MOJO_PER_WHEAT;
  }
  if (height < 9 * BLOCKS_PER_YEAR) {
    return 0.5 * MOJO_PER_WHEAT;
  }
  if (height < 12 * BLOCKS_PER_YEAR) {
    return 0.25 * MOJO_PER_WHEAT;
  }

  return 0.125 * MOJO_PER_WHEAT;
}
...
function FarmingRewardsCards() {
  ...
  const estimatedDailyWHEATCard = React.useMemo(() => {
    if (fullNodeState !== FullNodeState.SYNCED || !expectedTimeToWinSeconds || !data) {
      const state = fullNodeState === FullNodeState.SYNCHING ? State.WARNING : undefined;

      return <FarmCardNotAvailable title={<Trans>Estimated daily WHEAT</Trans>} state={state} />;
    }

    const estimatedDailyWHEAT = new BigNumber(86_400)
    ...
    return (
      <CardSimple
        title={<Trans>Estimated daily WHEAT</Trans>}
        value={
          <>
            {mojoToWheatLocaleString(estimatedDailyWHEAT, locale)}
            &nbsp;
            {currencyCode}
          </>
        }
  ...
  const estimatedMonthlyWHEATCard = React.useMemo(() => {
    if (fullNodeState !== FullNodeState.SYNCED || !expectedTimeToWinSeconds || !data) {
      const state = fullNodeState === FullNodeState.SYNCHING ? State.WARNING : undefined;

      return <FarmCardNotAvailable title={<Trans>Estimated monthly WHEAT</Trans>} state={state} />;
    }

    const estimatedMonthlyWHEAT = new BigNumber(86_400 * 31)
      .div(expectedTimeToWinSeconds)
      .multipliedBy(getBlockRewardByHeight(data.peak.height))
      .dp(0);
    ...
    return (
      <CardSimple
        title={<Trans>Estimated monthly WHEAT</Trans>}
        value={
          <>
            {mojoToWheatLocaleString(estimatedMonthlyWHEAT, locale)}
            &nbsp;
            {currencyCode}
          </>
        }
  ...
  return (
    <Box>
      ...
      <Grid spacing={2} alignItems="stretch" container>
        ...
        <Grid xs={12} sm={6} md={4} item>
          {estimatedDailyWHEATCard}
        </Grid>
        <Grid xs={12} sm={6} md={4} item>
          {estimatedMonthlyWHEATCard}
        </Grid>
      </Grid>
    </Box>
  );
