import { ConfirmDialog, Flex } from '@wheat-network/core';
...
function getSpendableAmountUponUnlockingAssets(assetStatus: AssetStatusForOffer, assetsToBeUnlocked: PendingAsset[]) {
  ...
  for (let s = 0; s < assetsToBeUnlocked.length; s++) {
    ...
    if (lockedAsset.type === 'WHEAT' && assetStatus.type === 'WHEAT') {
      // assetId might be either 'WHEAT' or 'UNKNOWN'(maybe fee) but here both are treated as just a WHEAT spending
