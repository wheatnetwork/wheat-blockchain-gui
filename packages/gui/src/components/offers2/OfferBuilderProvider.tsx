import { fungibleAssetFromAssetIdAndAmount, royaltyAssetFromNFTInfo } from '@wheat-network/api';
import type { CalculateRoyaltiesRequest, NFTInfo } from '@wheat-network/api';
import {
  useCalculateRoyaltiesForNFTsQuery,
  useGetNFTsByNFTIDsQuery,
  useGetWalletsQuery,
} from '@wheat-network/api-react';
import { catToMojo, wheatToMojo } from '@wheat-network/core';
...
export default function OfferBuilderProvider(props: OfferBuilderProviderProps) {
  ...
  const requestedWHEAT = useWatch({
    name: 'requested.wheat',
  });

  const offeredWHEAT = useWatch({
    name: 'offered.wheat',
  });
  ...
  const requestedFungibleAssets = [
    ...(requestedWHEAT ?? [])
      ...
      .map(({ amount }) => fungibleAssetFromAssetIdAndAmount('wheat', wheatToMojo(amount))),
    ...(requestedTokens ?? [])
