import { useGetAutoClaimQuery, useSetAutoClaimMutation } from '@wheat-network/api-react';
import { Flex, SettingsText, Form, ButtonLoading, Fee, wheatToMojo, mojoToWheat } from '@wheat-network/core';
...
export default function SettingsCustodyAutoClaim(props) {
  ...
  const autoClaimFee = autoClaimData?.txFee ? mojoToWheat(autoClaimData.txFee).toNumber() : 0;
  ...
  async function handleSubmit({ fee }: { fee: number }) {
    const feeInMojos = wheatToMojo(fee);
    await setAutoClaim({
      enabled: fee > 0,
      txFee: feeInMojos,
