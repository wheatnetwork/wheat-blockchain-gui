...
export default function ProfileAdd() {
  ...
  const currencyCode = (useCurrencyCode() ?? 'WHEAT').toUpperCase();
  const isTestnet = currencyCode === 'TWHEAT';
  ...
  const spendableBalance = mojoToWheatLocaleString(balance?.spendableBalance);
              ...
              {!canCreateProfile && (
                <Flex flexDirection="column" gap={1}>
                  ...
                  <Link onClick={handleClick}>
                    <Trans>Get Mojos from the Wheat Faucet</Trans>
                  </Link>
                </Flex>
              )}
              <Flex flexDirection="column" gap={1}>
