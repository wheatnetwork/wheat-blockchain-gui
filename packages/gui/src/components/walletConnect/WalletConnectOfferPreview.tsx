import { Button, Flex, useOpenDialog, wheatToMojo } from '@wheat-network/core';
...
export default function WalletConnectOfferPreview(props: WalletConnectOfferPreviewProps) {
  ...
  async function handleShowPreview() {
    ...
    if (offerBuilderData) {
      // use new fee value
      const feeWheat = offerBuilderData.offered.fee?.[0]?.amount;
      if (feeWheat) {
        const feeMojos = feeWheat ? wheatToMojo(feeWheat).toFixed() : '0';
        onChange({
          ...values,
          fee: feeMojos,
        });
