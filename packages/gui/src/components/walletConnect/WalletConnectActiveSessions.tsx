import { Flex, Loading, TooltipIcon } from '@wheat-network/core';
...
const supportedCommands = WalletConnectCommands.map((item) => `wheat_${item.command}`);
        ...
        {isLoading ? (
          ...
          <Flex flexDirection="column" gap={2}>
            {pair.sessions.map((session) => {
              ...
              const methods = namespaces.wheat?.methods ?? [];
                ...
                <Flex flexDirection="column">
                  ...
                  {unsupportedMethods.length ? (
                    <Typography variant="body2" color="textSecondary">
                      ...
                      <TooltipIcon>
                        <Trans>
                          Your current version of the Wheat application may not support some commands requested by the
