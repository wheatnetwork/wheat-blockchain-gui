import { toBech32m, toSnakeCase } from '@wheat-network/api';
import { useSignMessageByAddressMutation, useSignMessageByIdMutation } from '@wheat-network/api-react';
import { Button, Card, Flex, Form, TextField, useOpenDialog, useShowError } from '@wheat-network/core';
  ...
  async function handleSignById(messageToSign: string, id: string, address: string) {
    ...
    if (error) {
      ...
      if (missingNFTMatch || missingDIDMatch) {
        ...
        const entityId = toBech32m(entityPuzzleHash, missingNFTMatch ? 'nft' : 'did:wheat:');
