import { usePwAbsorbRewardsMutation, useGetPlotNFTsQuery, useGetCurrentAddressQuery } from '@wheat-network/api-react';
import {
  UnitFormat,
  CardStep,
  ButtonLoading,
...
export default function PlotNFTAbsorbRewards(props: Props) {
  ...
  async function handleSubmit({ fee }: SubmitData) {
    try {
      ...
      const feeMojos = wheatToMojo(fee);
        ...
        <Flex flexDirection="column" gap={3}>
          ...
          >
            <Typography variant="subtitle1">
              <Trans>
                You will recieve{' '}
                <UnitFormat value={mojoToWheatLocaleString(balance)} display="inline" state={State.SUCCESS} /> to{' '}
                {address}
              </Trans>
            </Typography>
