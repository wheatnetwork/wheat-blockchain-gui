...
import type { CATToken, Wallet } from '@wheat-network/api';
import { useGetCatListQuery, useGetWalletsQuery } from '@wheat-network/api-react';
...
export default function NFTOfferTokenSelector(props: Props) {
  ...
  const [, options]: [TokenSelectOption, TokenSelectOption[]] = useMemo(() => {
    ...
    const wheatWalletSelection = [wallets.find((wallet: Wallet) => wallet.type === WalletType.STANDARD_WALLET)].map(
      (wallet: WalletType) => ({
        walletId: wallet.id,
        walletType: wallet.type,
        name: 'Wheat',
        symbol: currencyCode,
        displayName: `Wheat (${currencyCode})`,
        disabled: false,
        tail: '',
      })
    );
    ...
    const allOptions = [...wheatWalletSelection, ...catOptions];
