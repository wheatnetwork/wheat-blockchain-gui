import { OfferSummaryRecord, OfferTradeRecord } from '@wheat-network/api';
import { useCheckOfferValidityMutation, useGetWalletsQuery } from '@wheat-network/api-react';
import {
  Back,
  Button,
  ButtonLoading,
...
function NFTOfferSummaryRow(props: NFTOfferSummaryRowProps) {
  ...
  const assetIdsToTypes: { [key: string]: OfferAsset | undefined }[] = useMemo(
    () =>
      Object.keys(summaryData).map((key) => {
        ...
        if (['wheat', 'twheat'].includes(key.toLowerCase())) {
  ...
  const rows: (React.ReactElement | null)[] = assetIdsToTypes.map((entry) => {
    ...
    switch (assetType) {
      case undefined:
        return null;
      case OfferAsset.WHEAT: // fall-through
...
function NFTOfferDetails(props: NFTOfferDetailsProps) {
  ...
  const nftSaleInfo = useMemo(() => {
    ...
    const wheatMakerFee = mojoToWheat(makerFee);

    return {
      ...calculateNFTRoyalties(
        amount,
        parseFloat(wheatMakerFee),
  ...
  const overrideNFTSellerAmount =
    exchangeType === NFTOfferExchangeType.TokenForNFT
      ? assetType === OfferAsset.WHEAT
        ? wheatToMojo(nftSaleInfo?.nftSellerNetAmount ?? 0)
