...
import type { NFTInfo, Wallet } from '@wheat/api';
import {
...
/*        An NFT offer consists of a single NFT being offered for WHEAT         */
/* ========================================================================== */
          ...
          <Box style={{ position: 'relative', top: '20px' }}>
            <TooltipIcon>
              <Trans>
                ...
                0.000005 WHEAT (5,000,000 mojos)
              </Trans>
            </TooltipIcon>
          </Box>
        </Flex>
...
function NFTOfferConditionalsPanel(props: NFTOfferConditionalsPanelProps) {
  ...
  const amount = methods.watch('wheatAmount');
  ...
  const offerElem =
    tab === NFTOfferExchangeType.NFTForWHEAT ? nftElem : amountElem;
  const takerElem =
    tab === NFTOfferExchangeType.NFTForWHEAT ? amountElem : nftElem;
  ...
  function handleAmountChange(amount: string) {
    methods.setValue('wheatAmount', amount);
  }
  ...
  return (
    ...
    >
      ...
      >
        <Tab
          value={NFTOfferExchangeType.NFTForWHEAT}
          label={<Trans>NFT for WHEAT</Trans>}
          disabled={disabled}
        />
        <Tab
          value={NFTOfferExchangeType.WHEATForNFT}
          label={<Trans>WHEAT for NFT</Trans>}
          disabled={disabled}
        />
      </Tabs>
      <Grid container>
        <Flex flexDirection="column" flexGrow={1} gap={3}>
          ...
          {tab === NFTOfferExchangeType.WHEATForNFT && (
            <Flex flexDirection="column" gap={2}>
              ...
              <>
                <Flex flexDirection="column" gap={0.5}>
                  <Typography variant="body1" color="textSecondary">
                    {tab === NFTOfferExchangeType.NFTForWHEAT ? (
...
/* ========================================================================== */
/*                              NFT Offer Editor                              */
/*             Currently only supports a single NFT <--> WHEAT offer            */
/* ========================================================================== */
...
function buildOfferRequest(
  ...
  const baseMojoAmount: BigNumber = wheatToMojo(wheatAmount);
  const mojoAmount =
    exchangeType === NFTOfferExchangeType.NFTForWHEAT
      ? baseMojoAmount
      : baseMojoAmount.negated();
  const feeMojoAmount = wheatToMojo(fee);
  const nftAmount = exchangeType === NFTOfferExchangeType.NFTForWHEAT ? -1 : 1;
  const wheatWalletId = 1;
  ...
  return [
    {
      ...
      [wheatWalletId]: mojoAmount,
    },
    exchangeType === NFTOfferExchangeType.WHEATForNFT ? driverDict : undefined,
    // driverDict,
    feeMojoAmount,
  ];
}
...
export default function NFTOfferEditor(props: NFTOfferEditorProps) {
  ...
  function validateFormData(
    ...
    const { exchangeType, nftId, wheatAmount, fee } = unvalidatedFormData;
    ...
    } else if (!wheatAmount || wheatAmount === '0') {
  ...
  async function handleSubmit(unvalidatedFormData: NFTOfferEditorFormData) {
    ...
    const { exchangeType, launcherId, wheatAmount, fee } = formData;

    if (exchangeType === NFTOfferExchangeType.NFTForWHEAT) {
      const haveNFT =
