import { OfferTradeRecord } from '@wheat-network/api';
import {
  ButtonLoading,
  CopyToClipboard,
  DialogActions,
...
const log = debug('wheat-gui:offers');
...
const testnetDummyHost = 'offers-api-sim.wheatcoin.top';
...
enum HashgreenErrorCodes {
  ...
  MARKET_NOT_FOUND = 50_029, // Pairing doesn't exist e.g. WHEAT/RandoCoin
...
async function postToHashgreen(offerData: string, testnet: boolean): Promise<string> {
  ...
  if (statusCode === 200) {
    ...
    if (testnet) {
      return 'https://www.wheatcoin.top/offers';
    }
...
async function postToSpacescan(offerData: string, testnet: boolean): Promise<{ viewLink: string; offerLink: string }> {
  ...
  const requestOptions = {
    ...
    port: 443,
    path: `/api/offer/upload?coin=${testnet ? 'twheat' : 'wheat'}&version=1`,
  };
  const requestHeaders = {
    'Content-Type': 'application/json',
  };
