...
export function determineNFTOfferExchangeType(
  ...
  return nftOffered
    ? NFTOfferExchangeType.NFTForWHEAT
    : NFTOfferExchangeType.WHEATForNFT;
}
...
export function getNFTPriceWithoutRoyalties(
  ...
  // NFTs can only be exchanged for WHEAT currently
  const amountInMojos = offerAssetAmountForAssetId('wheat', summary);
  if (amountInMojos === undefined) {
    return undefined;
  }
  return mojoToWheat(amountInMojos).toNumber();
}
