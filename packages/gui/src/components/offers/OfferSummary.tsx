...
import { Flex, FormatLargeNumber, StateColor, TooltipIcon, mojoToWheat, mojoToCAT } from '@wheat-network/core';
...
export default function OfferSummary(props: Props) {
  ...
  const makerAmount =
    makerEntries.length > 0
      ? ['wheat', 'twheat'].includes(makerEntries[0][0].toLowerCase())
        ? mojoToWheat(makerEntries[0][1])
        : mojoToCAT(makerEntries[0][1])
      : undefined;
  const takerAmount =
    takerEntries.length > 0
      ? ['wheat', 'twheat'].includes(takerEntries[0][0].toLowerCase())
        ? mojoToWheat(takerEntries[0][1])
