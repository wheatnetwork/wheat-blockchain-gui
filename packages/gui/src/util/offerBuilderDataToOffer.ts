...
import { wheatToMojo, catToMojo } from '@wheat-network/core';
...
export default async function offerBuilderDataToOffer({
  ...
  const {
    offered: { wheat: offeredWheat = [], tokens: offeredTokens = [], nfts: offeredNfts = [], fee: [firstFee] = [] },
    requested: { wheat: requestedWheat = [], tokens: requestedTokens = [], nfts: requestedNfts = [] },
  } = data;

  const usedNFTs: string[] = [];

  const feeInMojos = firstFee ? wheatToMojo(firstFee.amount) : new BigNumber(0);

  const walletIdsAndAmounts: Record<string, BigNumber> = {};
  const driverDict: Record<string, Driver> = {};

  if (!allowEmptyOfferColumn) {
    const hasOffer = !!offeredWheat.length || !!offeredTokens.length || !!offeredNfts.length;
        ...
        if (assetId.toUpperCase() === 'WHEAT' || assetId.toUpperCase() === 'UNKNOWN') {
          ...
          if (idx > -1) {
            ...
            if (pendingOffers[idx].assetId.toUpperCase() !== assetId.toUpperCase()) {
              // Now we can distinguish that we have wheat spending which is only WHEAT, only Fee or both WHEAT and Fee
              pendingOffers[idx].assetId = 'WHEAT+FEE';
            }
          } else {
            pendingOffers.push({
              type: 'WHEAT',
  ...
  let pendingWheatOffer: AssetStatusForOffer | undefined;
  let pendingWheat = new BigNumber(0);
  if (offeredWheat.length > 0 || feeInMojos.gt(0)) {
    ...
    for (let i = 0; i < pendingOffers.length; i++) {
      ...
      if (po.type === 'WHEAT') {
        pendingWheatOffer = po;
        pendingWheat = po.lockedAmount;
        break;
      }
    }
    if (standardWallet) {
  ...
  // offeredWheat.length should be always 0 or 1
  const wheatTasks = offeredWheat.map(async (wheat) => {
    const { amount } = wheat;
    if (!amount || amount === '0') {
      throw new Error(t`Please enter an WHEAT amount`);
    }
    if (!standardWallet || !standardWalletBalance) {
      throw new Error(t`No standard wallet found`);
    }

    const mojoAmount = wheatToMojo(amount);
    ...
    const hasEnoughTotalBalance = spendableBalance.plus(pendingWheat).minus(feeInMojos).gte(mojoAmount);
    if (!hasEnoughTotalBalance) {
      throw new Error(t`Amount exceeds WHEAT total balance`);
    }

    if (pendingWheatOffer) {
      // Assuming offeredWheat.length is always less then or equal to 1
      pendingWheatOffer.spendingAmount = mojoAmount.plus(feeInMojos);
      pendingWheatOffer.spendableAmount = spendableBalance;
      pendingWheatOffer.confirmedAmount = new BigNumber(standardWalletBalance.confirmedWalletBalance);
      const hasEnoughSpendableBalance = spendableBalance.gte(pendingWheatOffer.spendingAmount);
      if (!hasEnoughSpendableBalance) {
        pendingWheatOffer.status = 'conflictsWithNewOffer';
      } else {
        pendingWheatOffer.status = 'alsoUsedInNewOfferWithoutConflict';
      }
    }
  });
  // Treat fee as wheat spending
  if (offeredWheat.length === 0 && feeInMojos.gt(0)) {
    if (!standardWallet || !standardWalletBalance) {
      throw new Error(t`No standard wallet found`);
    }
    ...
    if (!hasEnoughTotalBalance) {
      throw new Error(t`Fee exceeds WHEAT total balance`);
    }
    if (pendingWheatOffer) {
      pendingWheatOffer.spendingAmount = feeInMojos;
      pendingWheatOffer.spendableAmount = spendableBalance;
      pendingWheatOffer.confirmedAmount = new BigNumber(standardWalletBalance.confirmedWalletBalance);
      const hasEnoughSpendableBalance = spendableBalance.gte(pendingWheatOffer.spendingAmount);
      if (!hasEnoughSpendableBalance) {
  ...
  await Promise.all([...wheatTasks, ...tokenTasks, ...nftTasks]);

  // requested
  requestedWheat.forEach((wheat) => {
    const { amount } = wheat;
    ...
    walletIdsAndAmounts[wallet.id] = wheatToMojo(amount);
  });
  ...
  const prepareNftTasks = requestedNfts.map(async ({ nftId }) => {
    ...
    if (driver) {
      ...
      if (considerNftRoyalty && pendingWheatOffer) {
        ...
        if (royaltyPercentageStr) {
          ...
          const spendingWheat = pendingWheatOffer.spendingAmount.minus(feeInMojos);
          const newSpendingWheat = spendingWheat.multipliedBy(royaltyMultiplier).plus(feeInMojos);
          pendingWheatOffer.spendingAmount = newSpendingWheat;

          const hasEnoughSpendableBalance = pendingWheatOffer.spendableAmount.gte(pendingWheatOffer.spendingAmount);
          if (!hasEnoughSpendableBalance) {
            pendingWheatOffer.status = 'conflictsWithNewOffer';
          } else {
            pendingWheatOffer.status = 'alsoUsedInNewOfferWithoutConflict';
