...
export function getUserDataDir(): string {
  const wheatRootPath = getConfigRootDir();
  const appName = app.getName();
  const userDataDir = path.join(wheatRootPath, 'gui', appName);
  return userDataDir;
}

export function setUserDataDir(): void {
  const wheatRootUserDataPath = getUserDataDir();

  migrateUserDataIfNecessary();

  console.info(`Setting user data directory to ${wheatRootUserDataPath}`);
  app.setPath('userData', wheatRootUserDataPath);
}
...
export function migrateUserDataIfNecessary() {
  ...
  const leveldbDestPath = path.join(wheatRootUserDataPath, 'Local Storage', 'leveldb');
