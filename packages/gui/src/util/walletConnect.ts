...
const log = initDebug('wheat-gui:walletConnect');

const availableCommands = walletConnectCommands.map((command) => `wheat_${command.command}`);
...
export async function processSessionProposal(
  ...
  try {
    ...
    const requiredNamespace = requiredNamespaces.wheat;
    if (!requiredNamespace) {
      throw new Error('Missing required wheat namespace');
    }

    const { chains, methods } = requiredNamespace;
    const chain = chains.find((item) => ['wheat:testnet', 'wheat:mainnet'].includes(item));
    if (!chain) {
    ...
    const accounts = fingerprints.map((fingerprint) => `wheat:${instance}:${fingerprint}`);
