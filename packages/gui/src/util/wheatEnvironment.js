...
const PY_DIST_FILE = 'wheat_daemon';
const PY_FOLDER = '../../../wheat/daemon';
...
const getWheatVersion = () => {
  let version = null;
  const exePath = getExecutablePath('wheat');
  // first see if we can get a wheat exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or wheat is on the path
    try {
      version = childProcess
