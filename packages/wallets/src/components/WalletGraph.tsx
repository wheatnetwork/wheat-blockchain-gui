import { TransactionType, WalletType } from '@wheat-network/api';
import type { Transaction } from '@wheat-network/api';
import { useGetWalletBalanceQuery } from '@wheat-network/api-react';
import { mojoToWheat, mojoToCAT, blockHeightToTimestamp } from '@wheat-network/core';
  ...
  */
  ...
  const points = [
    {
      ...
      y: BigNumber.max(0, (walletType === WalletType.CAT ? mojoToCAT(start) : mojoToWheat(start)).toNumber()), // max 21,000,000 safe to number
      tooltip: (walletType === WalletType.CAT ? mojoToCAT(balance) : mojoToWheat(balance)).toString(), // bignumber is not supported by react
    },
  ];
  ...
  data.forEach((item) => {
    ...
    points.push({
      x: timestamp,
      y: BigNumber.max(0, (walletType === WalletType.CAT ? mojoToCAT(start) : mojoToWheat(start)).toNumber()), // max 21,000,000 safe to number
      tooltip: walletType === WalletType.CAT ? mojoToCAT(start) : mojoToWheat(start).toString(), // bignumber is not supported by react
    });
  });
