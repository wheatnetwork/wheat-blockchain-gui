...
import { Color, mojoToWheat, mojoToCAT, blockHeightToTimestamp } from '@wheat-network/core';
  ...
  */
  ...
  const points = [
    {
      ...
      y: BigNumber.max(
        0,
        ([WalletType.CAT, WalletType.CRCAT].includes(walletType) ? mojoToCAT(start) : mojoToWheat(start)).toNumber()
  ...
  data.forEach((item) => {
    ...
    points.push({
      ...
      tooltip: [WalletType.CAT, WalletType.CRCAT].includes(walletType)
        ? mojoToCAT(start)
        : mojoToWheat(start).toString(), // bignumber is not supported by react
    });
  });
