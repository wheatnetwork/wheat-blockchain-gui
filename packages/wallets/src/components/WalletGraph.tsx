...
import { useGetWalletBalanceQuery } from '@wheat/api-react';
import { TransactionType } from '@wheat/api';
import type { Transaction } from '@wheat/api';
import { mojoToWheat, blockHeightToTimestamp } from '@wheat/core';
  ...
  */
  ...
  const points = [
    {
      ...
      y: BigNumber.max(0, mojoToWheat(start)).toNumber(), // max 21,000,000 safe to number
      tooltip: mojoToWheat(balance).toString(), // bignumber is not supported by react
    },
  ];
  ...
  data.forEach((item) => {
    ...
    points.push({
      x: timestamp,
      y: BigNumber.max(0, mojoToWheat(start)).toNumber(), // max 21,000,000 safe to number
      tooltip: mojoToWheat(start).toString, // bignumber is not supported by react
    });
  });
