...
import { OfferSummaryRecord, OfferTradeRecord, OfferCoinOfInterest, WalletType } from '@wheat/api';
import { useCheckOfferValidityMutation, useTakeOfferMutation } from '@wheat/api-react';
import {
  colorForOfferState,
...
function OfferDetails(props: OfferDetailsProps) {
      ...
      field: (coin: OfferCoinOfInterest) => {
        return (
          <Typography variant="body2">
            <Flex flexDirection="row" flexGrow={1} gap={1}>
              {mojoToWheatLocaleString(coin.amount)}
            </Flex>
          </Typography>
        )
      },
    ...
    },
    {
      field: (coin: OfferCoinOfInterest) => {
        return (
          ...
          >
            <Link
              onClick={(event: React.SyntheticEvent) => handleLinkClicked(event, `https://alltheblocks.net/wheat/coin/${coin.parentCoinInfo}`)}
            >
              {coin.parentCoinInfo}
            </Link>
          </Tooltip>
    ...
    },
    {
      field: (coin: OfferCoinOfInterest) => {
        return (
          ...
          >
            <Link
              onClick={(event: React.SyntheticEvent) => handleLinkClicked(event, `https://alltheblocks.net/wheat/address/${toBech32m(
              coin.puzzleHash,
  ...
  async function handleAcceptOffer(formData: any) {
    const { fee } = formData;
    const feeInMojos = fee ? Number.parseFloat(wheatToMojo(fee)) : 0;
