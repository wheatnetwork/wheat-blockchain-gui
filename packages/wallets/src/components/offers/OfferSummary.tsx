...
import { type OfferSummaryRecord  } from '@wheat/api';
import {
  Flex,
  StateColor,
  mojoToWheat,
...
export default function OfferSummary(props: Props) {
  ...
  const makerAmount = makerEntries[0][0].toLowerCase() === 'wheat' ? Number(mojoToWheat(makerEntries[0][1])) : Number(mojoToCAT(makerEntries[0][1]));
  const takerAmount = takerEntries[0][0].toLowerCase() === 'wheat' ? Number(mojoToWheat(takerEntries[0][1])) : Number(mojoToCAT(takerEntries[0][1]));
