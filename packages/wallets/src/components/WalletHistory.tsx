import { WalletType, TransactionType, TransactionTypeFilterMode, toBech32m } from '@wheat-network/api';
...
import type { Row } from '@wheat-network/core';
...
const getCols = (type: WalletType, isSyncing, getOfferRecord, navigate, location) => [
    ...
    field: (row: Row, metadata) => {
      ...
      return (
        <>
          ...
          <strong>
            <FormatLargeNumber
              value={[WalletType.CAT, WalletType.CRCAT].includes(type) ? mojoToCAT(row.amount) : mojoToWheat(row.amount)}
            />
          </strong>
          &nbsp;
          {metadata.unit}
...

  {
    field: (row: Row, metadata) => (
      <>
        <strong>
          <FormatLargeNumber value={mojoToWheat(row.feeAmount)} />
        </strong>
        &nbsp;
        {metadata.feeUnit}
      </>
