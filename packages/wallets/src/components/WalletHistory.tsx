...
import { Card, CardKeyValue, CopyToClipboard, Flex, Loading, StateColor, TableControlled, toBech32m, useCurrencyCode, mojoToWheatLocaleString, mojoToCATLocaleString } from '@wheat/core';
import { useGetOfferRecordMutation, useGetSyncStatusQuery } from '@wheat/api-react';
import styled from 'styled-components';
import type { Row } from '@wheat/core';
import { WalletType, TransactionType } from '@wheat/api';
...
const getCols = (type: WalletType, isSyncing, getOfferRecord, navigate) => [
        ...
        <>
          ...
          <strong>
            {type === WalletType.CAT
              ...
              : mojoToWheatLocaleString(row.amount)}
          </strong>
          &nbsp;
          {metadata.unit}
        </>
  ...
  },
  {
    field: (row: Row, metadata) => (
      <>
        <strong>{mojoToWheatLocaleString(row.feeAmount)}</strong>
        &nbsp;
        {metadata.feeUnit}
      </>
    ),
