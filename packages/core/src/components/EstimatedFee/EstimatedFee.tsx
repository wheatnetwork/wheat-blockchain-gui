import { useGetFeeEstimateQuery } from '@wheat-network/api-react';
...
import mojoToWheatLocaleString from '../../utils/mojoToWheatLocaleString';
import Fee from '../Fee';
...
export enum FeeTxType {
  walletSendWHEAT = 'send_wheat_transaction',
...
export default function EstimatedFee(props: FeeProps) {
  ...
  const formattedEstimates: FormattedEstimate[] = useMemo(() => {
    ...
    return estList.map((estimate: number, i: number) => {
      const formattedEstimate = mojoToWheatLocaleString(estimate, locale);
  ...
  useEffect(() => {
    if (formattedEstimates && inputType === 'dropdown') {
      ...
      if (selectedTime) {
        ...
        if (estimate) {
          const wheatFee = mojoToWheatLocaleString(estimate.estimate, 'en-US');
          setSelectedValue(estimate.formattedEstimate);
          setValue(name, wheatFee);
        }
      }
    }
